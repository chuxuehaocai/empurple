...
import dev.naominet.empurple.script.ScriptRepository
...
object WebUiPanel {
    ...
    private fun handle(exchange: HttpExchange) {
        try {
            ...
            when (exchange.requestURI.path) {
                ...
                "/api/scripts" -> sendJson(exchange, 200, scripts())
    ...
    private fun scripts(): JSONObject = runBlocking {
        val counts = ScriptRepository.installations().groupingBy { it.scriptId }.eachCount()
        jsonOf("scripts" to ScriptRepository.scripts().map { script ->
            jsonOf(
                "id" to script.id,
                "name" to script.name,
                "version" to script.version,
                "published" to script.published,
                "installations" to (counts[script.id] ?: 0),
                "createdAt" to Instant.ofEpochMilli(script.createdAt).toString(),
                "updatedAt" to Instant.ofEpochMilli(script.updatedAt).toString()
            )
        })
    }
    ...
    private val PANEL_HTML = """
        ...
        </style></head><body><main><header class="mast"><div><div class="kicker">Local observatory / read only</div><h1>em<br>purple</h1></div><p><span class="pulse"></span><strong id="health">正在连接</strong><br>插件、脚本与缓存的本地观测面板。这里不执行命令，也不修改运行状态。</p></header><section class="grid"><article class="card"><div class="label">Framework</div><div class="value" id="version">—</div><div class="muted" id="transport">等待状态</div></article><article class="card"><div class="label">Plugins</div><div class="value" id="pluginCount">—</div><ul id="plugins"></ul></article><article class="card"><div class="label">Cache</div><div class="value" id="cacheSize">—</div><div class="muted" id="cacheFiles">等待状态</div></article><article class="card wide"><div class="label">Published scripts</div><ul id="scripts"><li class="muted">正在读取</li></ul></article><article class="card"><div class="label">Panel contract</div><div class="value">GET</div><div class="muted">只读接口 · 不缓存 · 默认仅回环访问</div></article></section><footer id="updated">Empurple Panel</footer></main>
        <script>const api=p=>fetch(p).then(r=>{if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}),esc=v=>String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),bytes=n=>n<1024?n+' B':n<1048576?(n/1024).toFixed(1)+' KiB':(n/1048576).toFixed(1)+' MiB';Promise.all([api('/api/health'),api('/api/status'),api('/api/plugins'),api('/api/scripts'),api('/api/cache')]).then(([h,s,p,x,c])=>{health.textContent='运行正常 · '+Math.floor(h.uptimeMillis/1000)+'s';version.textContent=s.frameworkVersion;transport.textContent=s.protocol+' · '+s.address+':'+s.port+s.path;pluginCount.textContent=p.plugins.length;plugins.innerHTML=p.plugins.map(v=>'<li class="row"><code>'+esc(v.id)+'</code><span class="badge">loaded</span></li>').join('')||'<li class="muted">没有已加载插件</li>';const published=x.scripts.filter(v=>v.published);scripts.innerHTML=published.map(v=>'<li class="row"><span><code>/'+esc(v.name)+'</code><br><small class="muted">'+esc(v.id)+'</small></span><span class="badge">v'+v.version+' · '+v.installations+'</span></li>').join('')||'<li class="muted">没有已发布脚本</li>';cacheSize.textContent=bytes(c.totalBytes);cacheFiles.textContent=c.fileCount+' 个数据文件';updated.textContent='最后读取 '+new Date().toLocaleString()}).catch(e=>{health.textContent='读取失败';health.className='error';updated.textContent=e.message+' · 检查访问令牌'})</script></body></html>
    """.trimIndent()
}
